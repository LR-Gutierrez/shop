    ...
    <div class="container container-xxl col-md-12">
        <div class="bd-content">
            <div class="info content">
                <h1>Compremos algo!</h1>
                @if (session('status'))
                    <div class="alert alert-success">
                        {{ session('status') }}
                        @if (session('purchase'))
                            <p>Deseas ver tu <a href='{{ route('factura.generateInvoice', session('purchase')) }}' target="_blank">factura</a>?</p>
                        @endif
                    </div>
                @endif
            </div>
        ...
        <div class="modal fade" id="buyModal" tabindex="-1" aria-labelledby="buyLabel" aria-hidden="true">
            <div class="modal-dialog">
                ...
                <div class="modal-footer">
                    ...
                    <a id="buttonConfirm" onclick="redirect()" href="{{ route('productos.comprar', ["idproducto"=>$product->id]) }}"><button type="button" class="btn btn-success">Confirmar</button></a>
                </div>
                </div>
            </div>
        </div>
